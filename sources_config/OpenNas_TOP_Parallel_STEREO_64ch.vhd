...
architecture OpenNas_arq of OpenNas_Parallel_STEREO_64ch is
	...
	signal spikes_in_left_pdm : std_logic_vector(SPIKE_BUS_BIT_WIDTH - 1 downto 0);
	...
	--Audio input modules out spikes signal
	signal spikes_in_right_i2s : std_logic_vector(SPIKE_BUS_BIT_WIDTH - 1 downto 0);
	signal spikes_in_right_pdm : std_logic_vector(SPIKE_BUS_BIT_WIDTH - 1 downto 0);
	...
	-- attribute MARK_DEBUG of pdm_reset, i2s_reset, 
	-- 						source_sel_n, 
	-- 						spikes_in_left_i2s, spikes_in_left_pdm, spikes_in_left, spikes_out_left,
	-- 						spikes_in_right_i2s, spikes_in_right_pdm, spikes_in_rigth, spikes_out_rigth,
	-- 						spikes_out : signal is "TRUE";
...
begin
	...
	U_PFBank_2or_64CH_Left : entity work.PFBank_64CH
		generic map(
			CONFIG_ADDRESS => 16#0009#,
			CONFIG_OFFSET  => 255       -- Don't change this value
		)
		Port Map(
			clock       => clock,
			rst_n       => rst_n,
	...
	U_PFBank_2or_64CH_Rigth : entity work.PFBank_64CH
		generic map(
			CONFIG_ADDRESS => 16#0109#,
			CONFIG_OFFSET  => 255       -- Don't change this value
		)
		Port Map(
			clock       => clock,
